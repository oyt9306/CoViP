...
class VLMGSPOTrainer(Trainer):
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        seq_lens = completion_mask.sum(dim=1)
        ...
        # advantages = (rewards - mean_grouped_rewards) # 251114 ysw edited: GRPO to Dr. GRPO
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # 1) per-token log prob는 그대로 구함
        per_token_logps = self._get_per_token_logps(model, input_ids, attention_mask, **multimodal_inputs)
        per_token_logps = per_token_logps[:, prompt_ids.size(1) - 1:]   # completion part만
        ...
        completion_mask = completion_mask   # 이미 위에서 가져온 것

        # 필요시, _generate_and_score_completions에서 넘겨준 seq_lens 사용
        seq_lens = inputs.get("seq_lens", completion_mask.sum(dim=1))  # (B,)

        # 2) 시퀀스별 log-likelihood 합 (prompt 제외, completion만)
        logp_new_seq = (per_token_logps * completion_mask).sum(dim=1)        # (B,)
        logp_old_seq = (old_per_token_logps * completion_mask).sum(dim=1)    # (B,)

        # 3) 길이 정규화된 log-ratio → 시퀀스 importance ratio s_i
        log_ratio = (logp_new_seq - logp_old_seq) / seq_lens
        seq_ratio = torch.exp(log_ratio)   # s_i(θ)

        # 4) PPO-style clipping을 시퀀스 단위로
        seq_ratio_clipped = torch.clamp(seq_ratio, 1 - self.epsilon_low, 1 + self.epsilon_high)

        loss1 = seq_ratio * advantages          # (B,)
        loss2 = seq_ratio_clipped * advantages  # (B,)
        seq_loss = -torch.min(loss1, loss2)     # (B,)

        if self.beta > 0:
            ref_per_token_logps = inputs["ref_per_token_logps"]   # (B, L_c)
            ...
            # 토큰 마스크로 가리고, 평균을 내서 "시퀀스 KL"로 만들기
            seq_kl = (per_token_kl * completion_mask).sum(dim=1) / seq_lens    # (B,)

            # GSPO sequence loss에 KL penalty를 더함
        ...
        with torch.no_grad():
            # per_token_logps: (B, L_c), log π(a_t | s_t)
            per_token_entropy = -per_token_logps  # (B, L_c)
            # 시퀀스별 평균 엔트로피
            seq_entropy = (per_token_entropy * completion_mask).sum(dim=1) / completion_mask.sum(dim=1)
        ...
        loss = seq_loss.mean()
        is_clipped = (seq_ratio != seq_ratio_clipped).float()   # (B,)
