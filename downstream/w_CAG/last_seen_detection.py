...
def main():
    ...
    dataset = load_dataset(data_path) # load_from_disk("your_dataset_name")
    ...
    for i in tqdm(range(0, data_size, batch_size)):
        ...
        for j in range(i, i + bs):
            user_content = []
            lsd_imgs = dataset[j]["images_context"]
            lsd_dials = dataset[j]["dialogues_context"]
    ...
    print("Generating responses for LSD task...")
    responses = []
    for i in tqdm(range(0, data_size, batch_size)):
        inputs = []
        bs = min(batch_size, data_size - i)
        for j in range(i, i + bs):
            ...
            messages = [
                ...
                {
                    "role": "user",
                    "content": "Where did I last see the one in the new image? Think step by step and provide your final answer within //boxed{your_answer}.",
                },
            ]
            inputs += [
                prepare_inputs_for_vllm(message, processor) for message in [messages]
    ...
    print(f"  Perfect matches (F1=1.0): {sum(1 for f1 in f1_scores if f1 == 1.0)}")
    print(f"  Partial matches (0<F1<1): {sum(1 for f1 in f1_scores if 0 < f1 < 1.0)}")
    print(f"  No matches (F1=0.0): {sum(1 for f1 in f1_scores if f1 == 0.0)}")

    save_path = f"lsd_responses_{model_id.split('/')[-1]}.json"
