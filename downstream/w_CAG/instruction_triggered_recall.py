...
def main():
    ...
    print("Generating responses for ITR task...")
    responses = []
    for i in tqdm(range(0, data_size, batch_size)):
        inputs = []
        bs = min(batch_size, data_size - i)
        for j in range(i, i + bs):
            ...
            messages = [
                ...
                {
                    "role": "user",
                    "content": "Where did I last see the one in the new image?",
                },
            ]
            inputs += [
                prepare_inputs_for_vllm(message, processor) for message in [messages]
    ...
    for r in responses:
        if "SKS" in r:
            ...
            scores.append(0)
    print(f"ITR Score (w/ CAG): {np.mean(scores).item():.4f}")

    save_path = f"itr_responses_{model_id.split('/')[-1]}.json"
